/**
 * Lab Field UI (display) — pick a type, edit only that control.
...
export default function Edit( { attributes, setAttributes } ) {
	...
	const blockProps = useBlockProps( {
		className: `lab-field-ui lab-field-ui--${ fieldType }`,
	} );
	...
	switch ( fieldType ) {
		...
		case 'checkbox':
			control = (
				<div className="lab-field-ui__checkbox-group">
					<p className="lab-field-ui__control-label">{ fieldLabel }</p>
		...
		case 'image':
			control = (
				<div className="lab-field-ui__media">
					<p className="lab-field-ui__control-label">{ fieldLabel }</p>
					<MediaUploadCheck>
						<MediaUpload
	...
	return (
		<>
			<InspectorControls>
				<PanelBody
					title={ __( 'Content type', 'gutenberg-lab' ) }
					initialOpen={ true }
				>
					<SelectControl
						label={ __( 'What to display?', 'gutenberg-lab' ) }
						help={ __(
							'Frontend shows this value as content only (not a form input).',
							'gutenberg-lab'
						) }
						value={ fieldType }
						options={ FIELD_TYPES }
				...
				{ needsChoices && (
					<PanelBody
						...
						initialOpen={ true }
					>
						<p className="lab-field-ui__choices-help">
							{ __(
								'Add choices, then pick which ones to show on the frontend.',
								'gutenberg-lab'
							) }
						</p>
						{ choiceOptions.map( ( item, index ) => (
							<div
								className="lab-field-ui__choice-row"
			...
			<div { ...blockProps }>
				<div className="lab-field-ui__type-picker">
					<SelectControl
						label={ __( 'Content type', 'gutenberg-lab' ) }
						value={ fieldType }
						options={ FIELD_TYPES }
						onChange={ onChangeFieldType }
					/>
				</div>
				<div className="lab-field-ui__badge">
					{ TYPE_LABELS[ fieldType ] || fieldType }
				</div>
				<div className="lab-field-ui__control">{ control }</div>
				<p className="lab-field-ui__preview-line">
					<strong>{ __( 'Display:', 'gutenberg-lab' ) }</strong>{ ' ' }
					{ fieldType === 'image' && imageUrl ? (
						<img
							className="lab-field-ui__thumb"
							src={ imageUrl }
							alt={ imageAlt }
						/>
					) : (
